...
fn process_in_progress_transaction(tx: &Transaction<Committed>) {
    println!(
...
fn rollback_in_progress(tx: &Transaction<RolledBack>) {
    println!(
        "Rollback in progress transaction {} id", tx.id
    )
}
...
fn main() {
    ...
    let tx = Transaction::<InProgress>::new(2);
    let tx = tx.rollback();
    rollback_in_progress(&tx);
}
